import argparse
import random
from dataloader.dataset import MedicalDataSets,IristDataSets_SKI
import numpy as np
import torch
...
from network.SRS_S import SRS_S
...
parser.add_argument('--train_file_dir', type=str, default="bus_train.txt", help='dir')
parser.add_argument('--val_file_dir', type=str, default="bus_val.txt", help='dir')
parser.add_argument('--base_lr', type=float, default=0.01,
                    help='segmentation network learning rate')
parser.add_argument('--batch_size', type=int, default=8,
                    help='batch_size per gpu'),
parser.add_argument('--Dataset', type=str, default="KBT2019",
...
#                          train_file_dir=args.train_file_dir, val_file_dir=args.val_file_dir)
    db_train = IristDataSets_SKI(base_dir=args.base_dir, split="train",
                            transform=train_transform, train_file_dir=args.train_file_dir, val_file_dir=args.val_file_dir,DatasetChoice = args.Dataset)
    db_val = IristDataSets_SKI(base_dir=args.base_dir, split="val", transform=val_transform,
                          train_file_dir=args.train_file_dir, val_file_dir=args.val_file_dir,DatasetChoice = args.Dataset)
...
#    model_R.load_state_dict(torch.load(os.path.join('/mnt/sda/pai-hdd/bknian/开源_code/SRS/log_pth/BUS3_SRS_R.pth')))
#    model_S = SRS_S(bs=args.batch_size).cuda(args.gpu)
#    model_S.load_state_dict(torch.load(os.path.join('/mnt/sda/pai-hdd/bknian/开源_code/SRS/log_pth/BUS3_SRS_S.pth')))
    model_R = SRS_R(bs=args.batch_size).cuda(args.gpu)
    model_R.load_state_dict(torch.load(os.path.join('/mnt/sda/pai-hdd/bknian/开源_code/SRS/log_pth/KBT2019_SRS_R_model.pth')))
    model_S = SRS_S(bs=args.batch_size).cuda(args.gpu)
    model_S.load_state_dict(torch.load(os.path.join('/mnt/sda/pai-hdd/bknian/开源_code/SRS/log_pth/KBT2019_SRS_S_model.pth')))
    print("已经加载预训练权重")
