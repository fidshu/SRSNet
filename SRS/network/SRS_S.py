...
from network.DPConv.DPConv import PKernel_D2
from network.Component.IPALC import IPATR
from network.Component.SPD import Spu
from network.Component.CALayer import ECA_Layer
...
#        self.PKernel_D2 = PKernel_D2(input_channel = nb_filter[3],output_channel = nb_filter[3],kernel_size = 3,batch_size=self.bs,infor_channel=nb_filter[3])
        self.PKernel_D2 = PKernel_D2(input_channel = nb_filter[3],output_channel = nb_filter[3],kernel_size = 3,batch_size=self.bs,infor_channel=nb_filter[1])
        ...
        self.finalconv = self._make_layer(block,nb_filter[0]*3,nb_filter[0])
        
        # self.mask_head_1x1 = PKernel_D2(input_channel = nb_filter[0],output_channel = 1,kernel_size = 1,batch_size=self.bs,infor_channel=16)
        self.mask_head_1x1 = nn.Conv2d(nb_filter[0], 1, kernel_size = 1, stride = 1)
    ...
    def forward(self,x,y3_0):
        ...
        f_3 = self.bottomuplocal_fpn_3((x2_1),self.UpSC(x3_0))
        f_2 = self.bottomuplocal_fpn_2((x1_2),self.UpSC(f_3))
        f_1 = self.bottomuplocal_fpn_1((x0_3),self.UpSC(f_2))

        
        final = torch.cat([f_1,self.up(self.conv0_1_1x1(f_2)),self.up_4(self.conv0_2_1x1(f_3))],1)
        final = self.finalconv(final)
        
        # pre   = self.mask_head_1x1(final,x0_0)
...
def SRS_S(bs = 4):
    ...
    print("hello SRS_S ")
    # return ResNetOutput(Dense_BasicBlock_MALC, [6, 8, 16, 8],bs)
    # return ResNetOutput(Dense_BasicBlock_MALC, [6, 8, 12, 6],bs)
    return ResNetOutput(Dense_BasicBlock_MALC, [3, 4, 6, 3],bs)