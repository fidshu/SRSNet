...
parser.add_argument('--base_dir', type=str, default="/mnt/sda/dataset/SRS_DataSet/KBT2019", help='dir')
...
def get_model(args):
    model_R = SRS_R(bs=args.batch_size).cuda()
    model_R.load_state_dict(torch.load(os.path.join(r"./pth/KBT2019_SRS_R_model.pth")))
    model_S = SRS_S(bs=args.batch_size).cuda()
    # model_S.load_state_dict(torch.load(os.path.join(r"./权重文件/KBT2019_SRS_S_model.pth")))
...
def train(args):
    ...
    for epoch_num in range(max_epoch):
        ...
        if avg_meters['val_iou'].avg > best_iou:
            torch.save(model_s.state_dict(), 'pth/KBT2019_SRS_S_model.pth')
